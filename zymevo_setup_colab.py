...
run_cmd("apt-get update -qq && apt-get install -y python2.7 csh", "Installing Python 2.7 and csh")
run_cmd("curl -s https://bootstrap.pypa.io/pip/2.7/get-pip.py | python2.7", "Installing pip2")
run_cmd("pip2 install numpy", "Installing numpy")

print_status("Step 3/6: Installing OpenBabel")
run_cmd("apt-get install -y openbabel python3-openbabel", "Installing OpenBabel")
...
if all(status == "✓" for _, status, _ in results):
    ...
    <div style="text-align:center; padding:15px; background-color:#e8f5e9; 
        ...
        <p>All components installed successfully. Ready for ZymEvo.</p>
    </div>
    """))
else:
    display(HTML("""
